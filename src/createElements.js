...
export function infoBoxUpdate (obj) {
    ...
    obj.tasks[id].description = getElement(
      "#description-info"
    ).value;
    obj.tasks[id].priority = getElement(
      "#priority-info"
    ).value;
    obj.tasks[id].notes = getElement("#notes-info").value;
    obj.tasks[id].dueDate =
...
export function createTodos (lastItem, index, obj) {
    ...
    p2.textContent = `Due date: ${
      lastItem.dueDate !== "not set" ? formatDistanceToNow(parseISO(lastItem.dueDate), { addSuffix: true }) : lastItem.dueDate}`;
    pPriority.textContent = (div.classList.toggle(lastItem.priority), `Priority: ${lastItem.priority}`);  // Color border according to priority
